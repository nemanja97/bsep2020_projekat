...
async function getBlacklists() {
  return await axios.get(
    `${process.env.REACT_APP_API_URL}/v1/blacklists`
  );
}

async function getBlacklistById(id) {
  return await axios.get(
    `${process.env.REACT_APP_API_URL}/v1/blacklists?id=${id}`
  );
}
...
async function updateBlacklists(blacklist) {
  return await axios.put(`${process.env.REACT_APP_API_URL}/v1/blacklists`, blacklist);
}
