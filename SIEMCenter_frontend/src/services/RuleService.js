...
async function getRules(query) {
  ...
  return await axios.get(
    `${process.env.REACT_APP_API_URL}/v1/rules?${url_query}`
  );
}

async function getRuleById(id) {
  return await axios.get(
    `${process.env.REACT_APP_API_URL}/v1/rules?ids=${id}`
  );
}
...
async function updateRules(rule) {
  return await axios.put(`${process.env.REACT_APP_API_URL}/v1/rules`, rule);
}

async function deleteRule(id) {
  return await axios.delete(
    `${process.env.REACT_APP_API_URL}/v1/rules?id=${id}`
  );
}
